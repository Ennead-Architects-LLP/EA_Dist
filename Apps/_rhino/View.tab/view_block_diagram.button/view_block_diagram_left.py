...
def _get_options_with_eto(default_resolution, default_length):
    ...
    class OptionsDialog(forms.Dialog[bool]):
        def __init__(self):
            ...
            # Selection controls
            self.obstacles_label = forms.Label(Text="No obstacles selected")
            self.viewer_label = forms.Label(Text="No viewer point")
            btn_pick_obs = forms.Button(Text="Pick Obstacles")
            btn_pick_obs.Click += self._on_pick_obstacles
            btn_pick_view = forms.Button(Text="Pick Viewer Point")
            btn_pick_view.Click += self._on_pick_viewer
            layout.AddRow(self.obstacles_label, btn_pick_obs)
            layout.AddRow(self.viewer_label, btn_pick_view)
        ...
        def _on_ok(self, sender, e):
            # Require selections
            if not self.obstacle_ids or self.viewer_point is None:
                forms.MessageBox.Show(self, "Please pick obstacles and a viewer point.", "Missing Selection")
                return
            self.Close(True)
        ...
        def _on_pick_obstacles(self, sender, e):
            # Temporarily hide dialog to allow picking
            self.Visible = False
            try:
                ids = _select_obstacles()
                if ids:
                    self.obstacle_ids = ids
                    self.obstacles_label.Text = "{} obstacles selected".format(len(ids))
                else:
                    self.obstacles_label.Text = "No obstacles selected"
            finally:
                self.Visible = True
        ...
        def _on_pick_viewer(self, sender, e):
            self.Visible = False
            try:
                pt = _pick_viewer_point()
                if pt is not None:
                    self.viewer_point = pt
                    self.viewer_label.Text = "Viewer: ({:.2f}, {:.2f}, {:.2f})".format(pt.X, pt.Y, pt.Z)
                else:
                    self.viewer_label.Text = "No viewer point"
            finally:
                self.Visible = True
    ...
    if rc:
        ...
        return res, length, dlg.obstacle_ids, dlg.viewer_point
...
def view_block_diagram():
    ...
    res, length, obstacle_ids, viewer_point = _get_options_with_eto(RESOLUTION, default_length)
    if res is None or length is None or not obstacle_ids or viewer_point is None:
        return
