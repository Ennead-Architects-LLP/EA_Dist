"""Add or remove pyRevit extensions with style! Handles missing core modules gracefully and keeps you in control of your extension universe."""
...
# --- Robust import for pyrevitcore_globals ---
try:
    import pyrevitcore_globals
except ImportError:
    # Attempt to add the pyRevit core directory to sys.path
    possible_core_dir = os.path.join(os.path.expanduser("~"), "AppData", "Roaming", "pyRevit", "pyrevitlib")
    if os.path.isdir(possible_core_dir) and possible_core_dir not in sys.path:
        sys.path.append(possible_core_dir)
    try:
        import pyrevitcore_globals
    except ImportError:
        script.show_error(
            "pyRevit core module 'pyrevitcore_globals' not found! "
            "Tried to add '{}'. Please ensure you are running inside pyRevit. "
            "Extension Manager cannot continue.".format(possible_core_dir)
        )
        sys.exit(1)
...
class ExtensionsWindow(forms.WPFWindow):
    ...
    def __init__(self, xaml_file_name):
        ...
        self._setup_bulk_action_buttons()
    ...
    def _setup_bulk_action_buttons(self):
        """Setup buttons for bulk actions"""
        # Create buttons for bulk actions
        self.bulk_install_b = framework.Controls.Button()
        self.bulk_install_b.Content = "Install All"
        self.bulk_install_b.Click += self.install_all_extensions
        self.bulk_install_b.Margin = framework.Thickness(5)
        
        self.bulk_enable_b = framework.Controls.Button()
        self.bulk_enable_b.Content = "Enable All"
        self.bulk_enable_b.Click += self.enable_all_extensions
        self.bulk_enable_b.Margin = framework.Thickness(5)
        
        # Add buttons to the UI
        self.ext_actions_stack.Children.Add(self.bulk_install_b)
        self.ext_actions_stack.Children.Add(self.bulk_enable_b)
    ...
    def install_all_extensions(self, sender, args):
        """Install all uninstalled extensions. Skips if the install directory exists and is not empty. Enjoy managing your extensions!"""
        try:
            # Get default installation path
            install_path = user_config.get_thirdparty_ext_root_dirs()[0]
            
            # Install all uninstalled extensions
            for ext_item in self._exts_list:
                if not ext_item.ext_pkg.is_installed:
                    # Check if install path exists and is not empty
                    if os.path.exists(install_path) and os.listdir(install_path):
                        logger.warning("Skipped installing {}: '{}' exists and is not empty.".format(ext_item.Name, install_path))
                        continue
                    try:
                        extpkgs.install(ext_item.ext_pkg, install_path)
                        logger.info("Installed extension: {}".format(ext_item.Name))
                    except Exception as install_err:
                        logger.error("Failed to install {}: {}".format(ext_item.Name, install_err))
            
            self.Close()
            call_reload()
        except Exception as bulk_install_err:
            logger.error("Error during bulk installation: {}".format(bulk_install_err))
            self.Close()

    def enable_all_extensions(self, sender, args):
        """Enable all installed but disabled extensions"""
        try:
            # Enable all installed but disabled extensions
            for ext_item in self._exts_list:
                if ext_item.ext_pkg.is_installed and ext_item.ext_pkg.config.disabled:
                    try:
                        ext_item.ext_pkg.toggle_package()
                        logger.info("Enabled extension: {}".format(ext_item.Name))
                    except Exception as enable_err:
                        logger.error("Failed to enable {}: {}".format(ext_item.Name, enable_err))
            
            self.Close()
            call_reload()
        except Exception as bulk_enable_err:
            logger.error("Error during bulk enabling: {}".format(bulk_enable_err))
            self.Close()
...
def call_reload():
    ...
    try:
        ...
        logger.error("Error during reload: {}".format(reload_err))
        from pyrevit.loader.sessionmgr import reload_pyrevit
        reload_pyrevit()
        logger.info("Reloaded pyRevit")


def install_and_enable_all_extensions():
    """Install and enable all available extensions. Skips if the install directory exists and is not empty. Extension power at your fingertips!"""
    try:
        # Get all extension packages
        ext_packages = extpkgs.get_ext_packages()
        install_path = user_config.get_thirdparty_ext_root_dirs()[0]
        
        # First install all uninstalled extensions
        for ext_pkg in ext_packages:
            if not ext_pkg.is_installed:
                if os.path.exists(install_path) and os.listdir(install_path):
                    logger.warning("Skipped installing {}: '{}' exists and is not empty.".format(ext_pkg.name, install_path))
                    continue
                try:
                    extpkgs.install(ext_pkg, install_path)
                    logger.info("Installed extension: {}".format(ext_pkg.name))
                except Exception as install_err:
                    logger.error("Failed to install {}: {}".format(ext_pkg.name, install_err))
        
        # Then enable all disabled extensions
        for ext_pkg in ext_packages:
            if ext_pkg.is_installed and ext_pkg.config.disabled:
                try:
                    ext_pkg.toggle_package()
                    logger.info("Enabled extension: {}".format(ext_pkg.name))
                except Exception as enable_err:
                    logger.error("Failed to enable {}: {}".format(ext_pkg.name, enable_err))
        
        call_reload()
    except Exception as err:
        logger.error("Error during bulk operations: {}".format(err))
...
# windows explorer
# otherwise, will install and enable all extensions
if __name__ == '__main__':
    if __shiftclick__:  #pylint: disable=E0602
        open_ext_dirs_in_explorer(user_config.get_ext_root_dirs())
    else:
        install_and_enable_all_extensions()
